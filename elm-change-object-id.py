...
#
#    elm-change-object-id.py - Manipulate map object ID numbers
...
def position_to_tile(p):
    return floor(2 * p[0]), floor(2 * p[1])

def meshstr(i, e):
    n = e.name.strip(b'\x00').decode('ascii')
    p = e.position
    t = position_to_tile(p)
    return 'mesh %d:{"%s" @ %d,%d [%s] b=%d}' \
        % (i, n, t[0], t[1], vecstr(p), e.blend)

def make_dummy_mesh():
    m = MeshRecord()
    m.name = b'./3dobjects/badobject.e3d'
    m.blend = 20
    m.scale = 1
    return m

def change_object_id(m, o, n):
    l = m.mesh_list
    eo = l[o]
    while len(l) <= n:
        d = make_dummy_mesh()
    ...
    so = meshstr(o, eo)
    sn = meshstr(n, en)
    print('%s set to ID %d' % (so, n))
    print('  swapped with %s' % sn)

def dump_meshes(m):
    for i, e in enumerate(m.mesh_list):
        print(meshstr(i, e))
...
def main():
    ap = ArgumentParser(
        description='Change the ID number'
        ' of a 3D object in an Eternal'
        ' Lands map.',
        epilog='Mesh objects are printed as'
        ' "mesh ID:{E3D_PATH @ TILE_XY [WORLD_XYZ] b=BLEND}".'
        ' If BLEND is 20, the mesh is not displayed.'
    )
    ap.add_argument('map_file',
        help='The ELM file of the map'
        ' containing the object.',
    )
    ap.add_argument('-o', '--old-id', type=int,
        metavar='3D_MAP_OBJECT_ID',
        help='Current ID number of the object.',
    )
    ap.add_argument('-n', '--new-id', type=int,
        metavar='3D_MAP_OBJECT_ID',
        help='Set the object ID to this number.',
    )
    ap.add_argument('-b', '--no-backup',
        action='store_true',
        help='Do not create backup copies of maps.',
    )
    ap.add_argument('-d', '--dump',
        action='store_true',
        help='List all mesh objects in the map.',
    )
    a = ap.parse_args()
    m = Map(a.map_file)
    m.load()
    if a.dump:
        ...
        return
    if a.old_id is None:
        print('argument required: old object id (--old-id)')
        return
    if a.new_id is None:
        print('argument required: new object id (--new-id)')
        return
    change_object_id(m, a.old_id, a.new_id)
    if not a.no_backup:
        save_backup(m)
    m.save()
    print('wrote %s' % m.path)
